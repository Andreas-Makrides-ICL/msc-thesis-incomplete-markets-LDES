...
function solve_evaluation(set, param, evalu)
    ...
    for r in set.PROFILES, s in set.SHIFTERS
        evalu.UNSERVED_DEM[r,s] = (sum(param.DURATION[t]*(param.LOAD_FIX[t] -  value(model[:dem_served_fix][t,r,s])) for t in set.TIME_BLOCKS))
    end
...
INPUT_FPATH = "C:\\Users\\user\\Downloads\\Private-risk-and-social-resilience-in-liberalized-electricity-markets-Paper_code\\Han-shu-Private-risk-and-social-resilience-in-liberalized-electricity-markets-7a97a5a\\ercot_data.json"
