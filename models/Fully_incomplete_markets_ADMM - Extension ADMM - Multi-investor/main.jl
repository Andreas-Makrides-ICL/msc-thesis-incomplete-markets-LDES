...
function safeget(cap_dict, sym, key)
    sym ∈ keys(cap_dict) && key ∈ axes(cap_dict[sym]) ? cap_dict[sym][key] : 0.0
end
...
for delta in [1.0,0.8,0.6,0.4,0.2]#[1, 0.8, 0.6, 0.4, 0.2, 0.0] #[0.5] #[1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0.0]
    for psi in [0.5] #[0.5, 0.2, 0.1]
        ...
        push!(results, (
            ...
            Ψ = psi,
            objective = obj,
            PV = safeget(cap, :x_g_total, "PV"),
            Wind_Onshore = safeget(cap, :x_g_total, "Wind_Onshore"),
            Wind_Offshore = safeget(cap, :x_g_total, "Wind_Offshore"),
            Gas = safeget(cap, :x_g_total, "Gas"),
            Nuclear = safeget(cap, :x_g_total, "Nuclear"),
            BESS_P = safeget(cap, :x_P_total, "BESS"),
            BESS_E = safeget(cap, :x_E_total, "BESS"),
            Duration = safe_div(safeget(cap, :x_E_total, "BESS"), safeget(cap, :x_P_total, "BESS")),
            LDES_PHS_P = safeget(cap, :x_P_total, "LDES_PHS"),
            LDES_PHS_E = safeget(cap, :x_E_total, "LDES_PHS"),
            Duration_PHS = safe_div(safeget(cap, :x_E_total, "LDES_PHS"), safeget(cap, :x_P_total, "LDES_PHS")),
            # Multi-investor details
            Multi_PV = safeget(cap, :x_g, ("multi", "PV")),
            Multi_Gas = safeget(cap, :x_g, ("multi", "Gas")),
            Multi_Nuclear = safeget(cap, :x_g, ("multi", "Nuclear")),
            Multi_BESS_P = safeget(cap, :x_P, ("multi", "BESS")),
            Multi_BESS_E = safeget(cap, :x_E, ("multi", "BESS")),
            Multi_LDES_PHS_P = safeget(cap, :x_P, ("multi", "LDES_PHS")),
            Multi_LDES_PHS_E = safeget(cap, :x_E, ("multi", "LDES_PHS")),
        ))
...
CSV.write("ADMM_risk_aversion_results_O30_T672_multi_d.csv", df)
