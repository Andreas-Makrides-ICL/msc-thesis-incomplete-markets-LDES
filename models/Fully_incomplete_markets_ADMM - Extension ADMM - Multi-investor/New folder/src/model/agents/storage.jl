...
function define_storage!(model; remove_first::Bool=false, update_prices::Bool=false)
    ...
    if !update_prices
        ...
        @expression(m, stor_investment_costs[i in participants, s in S; s in get(setup["investor_storage_map"], i, [])], 
            (m[:x_P][i, s] / max(m[:x_P_total][s], 1e-6)) * (stor_data[s, "C_inv_P"] * stor_data[s, "CRF"] + stor_data[s, "FOMs"]) * m[:x_P][i, s] + 
    ...
    if !update_prices
        ...
        @constraint(m, LDES_PHS_morethan10h, m[:x_E_total]["H2"] >= 8.5 * m[:x_P_total]["H2"])
