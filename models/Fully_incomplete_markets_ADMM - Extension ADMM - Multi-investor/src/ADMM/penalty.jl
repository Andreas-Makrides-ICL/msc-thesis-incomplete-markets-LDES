...
function define_and_compute_penalty!(model, iteration)
    ...
    μ_g = Dict((g, o) => sum(dual for (i, gg, oo) in keys(model.results[iteration][:μ_g]) 
                                if gg == g && oo == o)
           for g in G, o in O)

    μ_s = Dict((s, o) => sum(dual for (i, ss, oo) in keys(model.results[iteration][:μ_s]) 
                                    if ss == s && oo == o)
            for s in S, o in O)
