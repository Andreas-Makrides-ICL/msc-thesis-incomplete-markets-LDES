...
function print_iteration(model, iter, total_time)
    ...
    # Print generator total capacities
    G = d["sets"]["G"]
    for g in G
        print(rpad(@sprintf("%.2f", get(results[:capacities][:x_g_total], g, 0.0)), data_width))
    end

    # Print storage (total Power & Energy) per tech
    S = d["sets"]["S"]
    for s in S
        print(rpad(@sprintf("%.2f", get(results[:capacities][:x_P_total], s, 0.0)), data_width))
        print(rpad(@sprintf("%.2f", get(results[:capacities][:x_E_total], s, 0.0)), data_width))
    end
    ...
    # Print total capacities and energy across all techs
    total_cap = sum(values(results[:capacities][:x_g_total])) + sum(values(results[:capacities][:x_P_total]))
    total_energy = sum(values(results[:capacities][:x_E_total]))
