...
function define_generator!(model; remove_first::Bool=false, update_prices::Bool=false)
    ...
    for sym in [:π_g, :ρ_g, :gen_profit_multi]
        maybe_remove_expression(m, sym)
    end

    @expression(m, x_g_total[g in G], sum(x_g[i, g] for i in participants if g in get(tech_map, i, [])))
    ...
    if !update_prices
        ...
        @expression(m, gen_investment_costs[i in participants, g in G; g in get(setup["investor_tech_map"], i, [])], 
            gen_data[g, "C_inv"] * gen_data[g, "CRF"] * m[:x_g][i, g]
        )
    ...
    for g in G
        if g == "Nuclear"
            @constraint(m, [t in T[2:end], o in O], 
                m[:q][g, t, o] - m[:q][g, t-1, o] <= ramp_rate * m[:x_g_total][g]
            )
            @constraint(m, [t in T[2:end], o in O], 
                m[:q][g, t-1, o] - m[:q][g, t, o] <= ramp_rate * m[:x_g_total][g]
            )
        end
    end
    ...
    for g in G
        if g == "Nuclear"
            @constraint(m, [t in T, o in O],
                m[:q][g, t, o] ≥ min_output_frac * m[:x_g_total][g]
            )
            @constraint(m, m[:x_g_total][g] ≤ nuclear_fraction * setup["peak_demand"])
        end
    end
    ...
    for g in G
        if g == "Gas"
            @constraint(m, m[:x_g_total][g] == gas_gen * setup["peak_demand"])
        end
    end
end
