...
function print_agents_objective_breakdown(m, str::String)
    ...
    averagetotalprice =sum(P[o] * W[t, o] * λ[t, o] for t in T, o in O) / sum(P[o] * W[t, o] for t in T, o in O)
    println("Total Average price = $(round(averagetotalprice, digits=4)) [£/MWh]")
    ...
    unserved_demand_fix = Dict(o => sum(W[t,o] *  value(m.model[:unserved_fixed][t,o]) for t in T) for o in O)
    unserved_demand_flex = Dict(o => sum(W[t,o] *  value(m.model[:unserved_flex][t,o]) for t in T) for o in O)
    ...
    filename1 = "riskneutralconsumersdfix_dflex_$(round(δ, digits=2))_psi_$(round(Ψ, digits=2))_$(str).csv"
    ...
    filename2 = "riskneutralconsumersprices_$(round(δ, digits=2))_psi_$(round(Ψ, digits=2))_$(str).csv"
    rows = [(t, o, λ[t, o]) for t in T for o in O]
    df = DataFrame(rows, [:Time, :Scenario, :λ])
    CSV.write(filename2, df)
    println("Full dfix, dflex saved to '$filename2'")
    ...
    social_welfare_incomplete = δ * exp_term  + (1 - δ) * cvar_val - (δ) * co2
