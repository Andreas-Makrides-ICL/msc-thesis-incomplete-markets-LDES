...
function print_objective_breakdown(m::OptimizationModel)
    ...
    gas_price_scenarios = m.data["data"]["gas_price_scenarios"]
    ...
    for o in O
        ...
        for g in G
            for t in T
                q = value(m.model[:q][g, t, o])
                c_v = g == "Gas" ? (gen_data[g, "C_v"] + gas_price_scenarios[o] * 6.995) : gen_data[g, "C_v"]
                gvc += W[t, o] * c_v * q
            end
        end
...
function recalculate_and_print_individual_risks(model::OptimizationModel)
    ...
    gas_price_scenarios = data["data"]["gas_price_scenarios"]
    ...
    for g in G
        ...
        gvc = Dict(o => sum(W[t, o] * ( g == "Gas" ? (gen_data[g, "C_v"] + gas_price_scenarios[o] * 6.995) : gen_data[g, "C_v"])* value(m[:q][g, t, o]) for t in T) for o in O)
