...
function extract_capacities!(model)
    ...
    # Per-investor investments
    capacities[:x_g] = haskey(m, :x_g) ? value.(m[:x_g]) : Dict()
    capacities[:x_P] = haskey(m, :x_P) ? value.(m[:x_P]) : Dict()
    capacities[:x_E] = haskey(m, :x_E) ? value.(m[:x_E]) : Dict()

    # Total installed capacity across investors
    capacities[:x_g_total] = haskey(m, :x_g_total) ? value.(m[:x_g_total]) : Dict()
    capacities[:x_P_total] = haskey(m, :x_P_total) ? value.(m[:x_P_total]) : Dict()
    capacities[:x_E_total] = haskey(m, :x_E_total) ? value.(m[:x_E_total]) : Dict()
...
function extract_iteration_results!(model, name)
    ...
    if haskey(m, :cvar_tail_multi)
        iteration_results[:μ_multi] = Dict(o => dual(m[:cvar_tail_multi][o]) for o in O)
    end
