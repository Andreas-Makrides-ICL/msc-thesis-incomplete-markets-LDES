...
function run_ADMM(data, setup, solver, delta)
    ...
    if termination_status(m.model) == MOI.OPTIMAL
        ...
        filename = "H2_15000_075_agent_objective_breakdown_delta_$(round(delta, digits=2)).txt"
        str = "H2_15000_075"
...
results = []
for delta in [0.75]#[1, 0.8, 0.6, 0.4, 0.2, 0.0] #[0.5] #[1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0.0]
    for psi in [0.5] #[0.5, 0.2, 0.1]
        ...
        local_setup["max_iterations"] = 5000
        local_setup["penalty"] = 2
        local_setup["tolerance"] = 0.5
        ...
        data = load_data(local_setup, user_sets = Dict("O" => 1:2, "T" => 1:2));
...
CSV.write("H2_15000_075_ADMM_risk_aversion_results_O30_T672_new_final.csv", df)
