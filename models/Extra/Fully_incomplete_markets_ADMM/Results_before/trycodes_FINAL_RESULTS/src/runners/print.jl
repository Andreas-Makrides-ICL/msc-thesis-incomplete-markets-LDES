...
function print_agents_objective_breakdown(m, str::String)
    ...
    nnn1=Dict(s => sum((-dual_discharge[(s,t,o)]-dual_charge[(s,t,o)])*value(m.model[:x_P][s]) for t in T, o in O) for s in S)
    nnn2=Dict(s => sum((-dual_energy[(s,t,o)])*value(m.model[:x_E][s]) for t in T, o in O) for s in S)
    ...
    for s in S
        println("Storage $(s) : nn = $(nn[s]), nnn1 = $(nnn1[s]) , nnn2 = $(nnn2[s]) , nnn = $(nnn[s])")
