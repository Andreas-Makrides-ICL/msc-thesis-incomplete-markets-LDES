...
function print_agents_objective_breakdown(m, str::String)
    ...
    scarcity_rent_discharge = Dict(s => sum((dual_discharge[(s,t,o)]/ (δ * P[o] + dual_val_s[(s,o)]))* W[t,o]  for t in T, o in O) for s in S)
    scarcity_rent_charge = Dict(s => sum((dual_charge[(s,t,o)]/ (δ * P[o] + dual_val_s[(s,o)]))* W[t,o]  for t in T, o in O) for s in S)
    scarcity_rent_energy = Dict(s => sum((dual_energy[(s,t,o)]/ (δ * P[o] + dual_val_s[(s,o)]))* W[t,o]  for t in T, o in O) for s in S)
