...
function run_ADMM(data, setup, solver, delta)
    ...
    if termination_status(m.model) == MOI.OPTIMAL
        ...
        filename = "H2_15000_01_agent_objective_breakdown_delta_$(round(delta, digits=2)).txt"
        str = "H2_15000_01"
...
results = []
for delta in [0.1]#[1, 0.8, 0.6, 0.4, 0.2, 0.0] #[0.5] #[1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0.0]
    for psi in [0.5] #[0.5, 0.2, 0.1]
        ...
        local_setup["max_iterations"] = 20000
...
CSV.write("H2_15000_01_ADMM_risk_aversion_results_O30_T672_new_final.csv", df)
